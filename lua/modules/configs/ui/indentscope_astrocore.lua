  ...
  opts.autocmds.indentscope_disabled = {
    {
      ...
      callback = function(event)
        if vim.b[event.buf].minicursorword_disable == nil then
          ...
          if vim.tbl_contains(blankline_opts.filetype_exclude or M.ignore_filetypes, filetype) then
    ...
    },
    {
      ...
      callback = function(event)
        if vim.b[event.buf].minicursorword_disable == nil then
          ...
          local blankline_opts = require("astrocore").plugin_opts("indent-blankline.nvim")
          if vim.tbl_contains(blankline_opts.buftype_exclude or M.ignore_buftypes, buftype) then
            vim.b[event.buf].miniindentscope_disable = true
          end
        end
      end,
